...
static int _pid_compute(pid_params_t *pid)
{
	...
	float increment = pid->kp * (pid->error - pid->last_error) 
									+ pid->ki * (pid->error)
									+ pid->kd * (pid->error - 2 * pid->last_error + pid->second_last_error);
	
		// 计算增量式控制量
	// pid->output += pid->ki * (pid->error - pid->last_error) + pid->kp * pid->error;  // 使用误差增量更新输出


	pid->output += increment+pid->error*0.5;
	


//    // 预计算输出，检查是否饱和
//    float new_output = pid->output + increment;
//	
//	  // 抗积分饱和处理（条件积分）
//    if (new_output > pid->output_max || new_output < pid->output_min)
//    {
//        // 输出将饱和，只执行PD控制，不累加积分项
//        increment = pid->kp * (pid->error - pid->last_error)
//                  + pid->kd * (pid->error - 2*pid->last_error + pid->second_last_error);
//        new_output = pid->output + increment;
//    }
